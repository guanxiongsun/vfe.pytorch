...
- **Since the audit:** the whole pipeline is ported and verified against the legacy stack (evaluator, data paths, test driver, optimiser, LR schedule, training step, training loop, and STPN's model and training pipeline). **Inference reproduces exactly: M1 83.80 vs 83.82 (MAMBA), M1′ 85.150 vs 85.152 (STPN).** **MAMBA's training reproduces: M3 scores 84.06 vs 83.82 once trained on the published model's own schedule** (its first attempt, batch 8 throughout as the config reads, gave 83.16). **STPN's training reproduces the trajectory but lands 0.61 low: M3′ 84.54 vs 85.15**, with per-epoch losses within 0.6% of the original and a stable final model (epoch 8: 84.39), so run-to-run variance is the likely cause.
...
- [x] **M2: short MAMBA run on Isambard** (job 6625045; 500 iterations, 4 GH200s × 2 micro-steps = batch 8, seed 1466607766). `tools/analyze_train_log.py` summarises a log and checks every logged LR against the schedule; on the original MAMBA log it confirms all 822 entries.
  ...
  - ≈ 0.14 GPU-hours.
- [x] **M3: full MAMBA 6x training → AP50 84.06 with the published model's own schedule (original 83.82, +0.25: within the ±0.5 target).** Jobs 6652686 (epochs 1–3 at batch 4: 4 GH200s, no accumulation, 27,422 iterations per epoch, seed 1485688156, 1 h 58 min) and 6652687 (epochs 4–6 at batch 8 via 2 micro-steps, seed 1466607766, 2 h 10 min including evaluation); ≈17 GPU-hours. The resume reproduced the original's own messages: *"the iteration number is changed due to change of GPU number (4 -> 8) … resumed epoch 3, iter 41133"*.

  | AP50 | M3 (mixed schedule) | M3 first attempt (batch 8 throughout) | original | mixed − original |
  |---|---|---|---|---|
  | all | 84.06 | 83.16 | 83.82 | +0.25 |
  | fast | 66.89 | 65.82 | 65.29 | +1.60 |
  | medium | 84.07 | 83.11 | 83.82 | +0.25 |
  | slow | 89.33 | 88.21 | 89.51 | −0.18 |

  - **Epochs 4–6 overlay the original log**: mean losses within 0.5%, 0.3% and 0.8%, gradient norms within 0.7% (epoch 4: 2.794 vs 2.795), accuracy equal to two decimals, all 822 LRs on schedule. Per-class APs still move ±5 (antelope −5.3, red_panda +4.5, mean 1.4), the usual spread of a 30-class average.
  - **The first attempt (83.16) followed the config as published, batch 8 for all six epochs**, and was 0.66 low: half the SGD steps in epochs 1–3. Keeping both runs is the evidence that the gap was the schedule, not the port.
...
## Progress log
- **2026-09-18 (M3 passed)** — **MAMBA trained with vfe scores AP50 84.06 against the published 83.82**, once trained the way the published model actually was: epochs 1–3 at batch 4, then epochs 4–6 at batch 8. Epochs 4–6 overlay the original log to within 1% on every loss and 0.7% on gradient norms, and the resume printed the same iteration-rescaling message as the original. The earlier batch-8-throughout run (83.16) stands as the control. MAMBA's reproduction is complete: released checkpoint 83.80 vs 83.82 (M1), trained model 84.06 vs 83.82 (M3).
