...
- [x] **7a Model** — `STPNSwinTransformer` and `PromptedPatchMerging` (`vfe/models/backbones/swin.py`), `STPN` with the `AttentionPredictor` / `AveragePredictor` DVP predictors (`vfe/models/vid/stpn.py`). Prompt support went into the existing Swin modules instead of a 1,000-line copy: window attention and patch merging detect prompt tokens from the sequence length, so plain Swin runs its unchanged path (`parity_backbone` and `parity_detector` rerun: still pass). The released STPN checkpoint (Hugging Face, SHA-256 verified) loads with no missing or unexpected keys; 45.0M parameters, as legacy.
  ...
  - **Training-step parity:** `tools/checks/parity_train_step.py --config …stpn… --data-config …mamba…` (STPN's own training pipeline is 7b): two AdamW steps from the released weights on real batches, DropPath active, CPU. **All 1,773 artifacts pass**: losses ≤ 2.7e-6, every gradient ≤ 6.8e-6, AdamW moments ≤ 1e-5, updates ≤ 5e-4. The harness now handles any optimiser state, optional clipping and two numerical facts found on the way:
    - Swin's key bias, like MAMBA's `ref_fc_embed.bias`, has an exactly zero gradient (it shifts every logit in a query's row equally), so its third of `qkv.bias` is compared as roundoff.
    - Adam turns roundoff-sized gradients into lr-sized steps of arbitrary sign (one weight's entire update difference came from 8 of 147,456 elements with |g| < 1e-7), so AdamW updates are weighted by `v / (v + 1e-12)`. A hard cut-off first tried instead let single elements fall on different sides of it on the two stacks.
- [ ] **7b STPN training pipeline:** `AutoAugment`, multi-scale `SeqResize` (`multiscale_mode="value"`), `SeqRandomCrop` (`absolute_range`, `allow_negative_crop`), `SeqMaxSizePad`, `SeqResize2`. Parity: exact under fixed seeds, as 5d.
