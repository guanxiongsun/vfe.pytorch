...
- **Since the audit:** evaluator (5a), test-time data path (5b), test driver (5c), train-time data path (5d), optimiser and LR schedule (6a/6b) are ported and verified against the legacy stack. Next: the training step and loop (6c/6d); M1 waits on the data upload.
...
- [ ] **M1:** released MAMBA checkpoint on the full VID val set, run with vfe on Isambard → **AP50 83.8**, within the protocol's own run-to-run randomness (target ±0.2).
  - Ready: checkpoint staged and SHA-256-verified at `/projects/b5cs/vfe/checkpoints/released/`; job `tools/isambard/test_video.sbatch` (a `SMOKE=1` mode runs one video and the data-pipeline check on the GH200 first); Isambard `.venv` aligned with this machine (vfe editable; OpenCV 4.14.0.94 and numpy 2.5.3, replacing an OpenCV **5.0.0** that predated the `<5` pin and would have broken the pixel-exact pipeline guarantee). Waiting on the data upload and staging job.
  - Open question the smoke test answers: are JPEG decode and `cv2.resize` bit-identical on aarch64 (NEON) and x86 (AVX)? Same OpenCV version, different SIMD code.
- [x] **5d Train-time data path (MAMBA)** — `vfe/datasets/imagenet_vid.py` training mode (drops images without ground truth or under 32 px, groups by aspect ratio), `dataset_wrappers.ConcatDataset` (a list in `data.train`), `LoadAnnotations`/`SeqLoadAnnotations`, box resize and flip, `SeqDefaultFormatBundle`, `GroupSampler`/`DistributedGroupSampler`, `collate_video_train`. Parity: `tools/checks/parity_vid_train_data.py`, **91 artifacts bit-exact**: the image ids of VID (56,061) and DET (53,621) after filtering, group flags, `DistributedGroupSampler` indices (8 ranks, epochs 0 and 3), `GroupSampler` indices, and 8 whole VID training samples (both flip states, bilateral reference sampling, boxes on the image border) against mmdet's `collate` + `scatter`.
  ...
  - Left: **DET samples** (`--det`), once a few DET images are copied back from Isambard after staging. The data loader's per-worker seeding belongs to 6d.
