...
- [x] **5b Test-time data path** — `vfe/datasets/pipelines/` (loading, the `Seq*` transforms, formatting; a `PIPELINES` registry so data configs run unchanged), `vfe/datasets/imagenet_vid.py` (all four reference samplers, `__getitem__`), `vfe/datasets/collate.py`. Parity: `tools/checks/parity_vid_pipeline.py` on real val videos at 6 resolutions (12 samples, 90 decoded frames): **model-facing structure, every meta and every pixel bit-exact** against mmdet's `collate` + `scatter`.
  ...
  - Box resizing and real flips arrived with 5d; multi-scale training (STPN) raises `NotImplementedError` until Phase 7. The no-flip test path still draws `np.random.choice` once per sample, as the original did, so numpy's RNG stream stays aligned.
- [x] **5c Test driver** — `python -m vfe.cli.test` (single GPU, or `torchrun` on one node), `vfe/apis/test.py`, `vfe/datasets/samplers.py` (`DistributedVideoSampler`: whole videos per rank, results concatenated in rank order as the original's modified collection did), `vfe/utils/dist.py`. Parity: `tools/checks/parity_vid_test.py`, **the released checkpoint on the first val video (464 frames, real images, memory carried across the whole video): every frame's detections match the legacy stack**, largest difference 4.3e-5 of scale. Only detections scoring ≥ 1e-3 are compared. Unfiltered, two of 464 frames differed, by one detection scoring exactly the 1e-4 threshold and one NMS pick between near-duplicates scoring 0.00027. The port also ran faster (33 s vs 39 s).
- [x] **M1: released MAMBA checkpoint on the full VID val set, vfe on Isambard → AP50 83.80** (original log 83.82; target ±0.2). Job 6624919, 4 GH200s, 20 min (47.7 frames/s per GPU), after a one-video smoke job (6624918); ≈1.35 GPU-hours in total.

  | AP50 | vfe (4× GH200) | original (8× A100) | diff |
  |---|---|---|---|
  | all | 83.80 | 83.82 | −0.01 |
  | fast | 65.31 | 65.29 | +0.03 |
  | medium | 83.82 | 83.82 | −0.00 |
  | slow | 89.51 | 89.51 | +0.01 |

  - Per-class APs differ by 0.08 on average (largest: sheep −0.56), the spread the protocol's random memory sampling and a 4-way instead of 8-way video split produce.
  - **The data pipeline is bit-exact across architectures:** `parity_vid_pipeline.py` artifacts from the GH200 (aarch64, NEON) match the x86 legacy run on all 61 artifacts, every decoded, resized, normalised and padded pixel included.
  - This validates Phases 3–5 on real data at full scale: model, checkpoint loading, test pipeline, driver and evaluator.
- [x] **5d Train-time data path (MAMBA)** — `vfe/datasets/imagenet_vid.py` training mode (drops images without ground truth or under 32 px, groups by aspect ratio), `dataset_wrappers.ConcatDataset` (a list in `data.train`), `LoadAnnotations`/`SeqLoadAnnotations`, box resize and flip, `SeqDefaultFormatBundle`, `GroupSampler`/`DistributedGroupSampler`, `collate_video_train`. Parity: `tools/checks/parity_vid_train_data.py`, **91 artifacts bit-exact**: the image ids of VID (56,061) and DET (53,621) after filtering, group flags, `DistributedGroupSampler` indices (8 ranks, epochs 0 and 3), `GroupSampler` indices, and 8 whole VID training samples (both flip states, bilateral reference sampling, boxes on the image border) against mmdet's `collate` + `scatter`.
  - **The sampler's length per rank is 13,711, the iterations per epoch in the original logs**, so the local annotation files filter to the same training set the published models saw.
  - Mutation-tested: an off-by-one in the flip, the sampler seed, or the clipping bound on either axis each fails the check. The clipping mutations needed samples chosen for it: VID boxes end at most at `size - 1`, so only a downscaled frame with a box on the edge can catch them.
  - mmcv's `scatter` to the CPU (`[-1]`) unsqueezes every tensor (`img` becomes `(1, 1, 3, H, W)`); scattering to a GPU, as training does, does not. The legacy check unwraps `DataContainer.data[0]`, the single device chunk that one-GPU scatter selects, so it reproduces the training shapes without requiring CUDA; `collate_video_train` produces the same shapes directly.
  - Harness-only shim: the legacy DET loader calls `mmpycocotools` aliases that official pycocotools lacks, so it is pointed at mmdet's own alias subclass.
  - **DET samples too** (`--det`, 121 artifacts bit-exact), with 116 DET images copied back from the staged data into the git-ignored `data/ILSVRC/Data/DET`. Loader-level parity (worker seeding) is in 6d.
...
- [x] **6c Full training-step parity** — `tools/checks/parity_train_step.py`: two consecutive CPU SGD steps from the released checkpoint on two real VID training batches. **All 1,899 artifacts pass**: exact batch digests, trainable/gradient-bearing parameter sets, RoI sampler choices and frozen buffers; losses, key/reference proposals, pre-clip gradient norm, every parameter gradient, momentum buffer and parameter update. Largest ordinary gradient difference is 1.31e-4 relative (tolerance 1e-3), proposals about 5e-8, and updates 7.48e-4; the four `ref_fc_embed.bias` gradients are the already-proven softmax shift-invariance roundoff case (largest absolute element 3.71e-10). This closes the full-model backward gap and verifies the second step's momentum state, not merely a single update.
- [x] **6d Training loop** — `python -m vfe.cli.train` (`vfe/cli/train.py`, `vfe/apis/train.py`), `vfe/datasets/loader.py`, `vfe/engine/{checkpoint,train_log}.py`. mmcv's `EpochBasedRunner` and hooks in their order (LR updates, sampler epochs, clipping, logging, checkpoints every `interval` epochs and after the last, evaluation), DDP via `torchrun`, mmcv-format checkpoints and text/JSON logs, `--cfg-options` with list indices as mmcv had them, and TF32 on by default because the original PyTorch 1.10 run on A100s had it.
  - **Gradient accumulation reproduces the 8-GPU run on fewer GPUs, exactly.** Each process runs one loader per *virtual rank*, built as the original rank's loader was (sampler rank, worker seeds), and keeps one torch generator state per virtual rank, swapped in around its micro-step and its iterator creation; losses are divided by `accumulate` and DDP syncs on the last micro-step.
  - Checks, all run locally:
    - `tools/checks/parity_train_loader.py`: vfe's virtual-rank loaders (4 processes × 2 micro-steps) against mmdet's real `build_dataloader` as ranks 0, 5 and 7 of 8, with 4 worker processes: **48 batches bit-exact** over two epochs, DET samples included.
...
- [x] **M2: short MAMBA run on Isambard** (job 6625045; 500 iterations, 4 GH200s × 2 micro-steps = batch 8, seed 1466607766). `tools/analyze_train_log.py` summarises a log and checks every logged LR against the schedule; on the original MAMBA log it confirms all 822 entries.
  ...
  - ≈ 0.14 GPU-hours.
- [ ] **M3: full MAMBA 6x training → AP50 83.16, 0.66 below the original 83.82 (target ±0.5: missed).** Job 6625056, the config as published (batch 8 for all six epochs, via 4 GH200s × 2 micro-steps), seed 1466607766; 4 h 14 min, ≈17 GPU-hours. Every logged LR matched the schedule; 0.168 s per iteration, 5,316 MiB.

  | AP50 | M3 | original | diff |
  |---|---|---|---|
  | all | 83.16 | 83.82 | −0.66 |
  | fast | 65.82 | 65.29 | +0.53 |
  | medium | 83.11 | 83.82 | −0.71 |
  | slow | 88.21 | 89.51 | −1.30 |

  - **Why, most likely:** the published model did not follow this schedule. Its epochs 1–3 ran at batch 4 on 4 GPUs (82,266 SGD steps; see the Caveat under Reproduction facts), twice as many steps as M3's batch 8. M3's per-epoch losses sit above the original log's throughout: +25% in epoch 4, +15% in epochs 5 and 6, gradient norms +30%. The evaluation protocol's own noise is far smaller (M1: 0.01 on the released model), and STPN's run on the same loop tracks its original log within ~1%.
  - **Next, if approved (~17 GPU-hours):** the original's schedule. Epochs 1–3 on 4 GPUs without accumulation (batch 4, 27,422 iterations per epoch, seed 1485688156, `--max-epochs 3`), then resume in the same work dir with `ACCUMULATE=2` (batch 8, seed 1466607766) for epochs 4–6. The loop now rescales the iteration count on such a resume, as mmcv did.

### Phase 7 — STPN
- [x] **7a Model** — `STPNSwinTransformer` and `PromptedPatchMerging` (`vfe/models/backbones/swin.py`), `STPN` with the `AttentionPredictor` / `AveragePredictor` DVP predictors (`vfe/models/vid/stpn.py`). Prompt support went into the existing Swin modules instead of a 1,000-line copy: window attention and patch merging detect prompt tokens from the sequence length, so plain Swin runs its unchanged path (`parity_backbone` and `parity_detector` rerun: still pass). The released STPN checkpoint (Hugging Face, SHA-256 verified) loads with no missing or unexpected keys; 45.0M parameters, as legacy.
  - Quirk kept deliberately: prompts are repeated over windows in (window, batch) order while windows are laid out in (batch, window) order. The two agree only for a batch of one, which is the only batch STPN runs with prompts; the released weights were trained with it.
  - **Inference parity:** `tools/checks/parity_vid_test.py --config configs/vid/stpn/stpn_swint_adam_9x.py` (the harness now takes any config): the released checkpoint over the first val video, 464 frames with per-video prompts, **every frame's detections match, largest difference 5.2e-7** of scale (1,393 artifacts; GPU).
  - **Training-step parity:** `tools/checks/parity_train_step.py --config configs/vid/stpn/stpn_swint_adam_9x.py`: two AdamW steps from the released weights on real STPN training batches (multi-scale and cropped), DropPath active, CPU. **All 2,209 artifacts pass**: losses ≤ 3.9e-5, every gradient and AdamW moment ≤ 4.4e-4, proposals ≤ 5.7e-8, every parameter's hyperparameters exact, and every update within 0.51 float32 spacings of AdamW's formula. Getting there changed how the harness judges optimiser steps, for both models:
    - Swin's key bias, like MAMBA's `ref_fc_embed.bias`, has an exactly zero gradient (it shifts every logit in a query's row equally), so its third of `qkv.bias` is checked as roundoff.
    - **Updates are no longer compared across stacks.** Gradient sums over thousands of tokens differ between the stacks by ~1e-6 absolute; Adam normalises each element, so elements within that noise take steps of different size or sign (one FFN bias: 2.7e-2 relative update difference from a single element with |g| ≈ 1e-6, while its gradient matched to 9e-5). Updates on weights near 1 are also quantised by float32 (~1e-7 increments for ~1e-5 steps). Instead, optimiser *state* is compared across stacks, and on each stack the change must equal the optimiser's formula (SGD: `-lr * momentum_buffer`; AdamW: decoupled decay and bias-corrected moments) applied to its own state, to within one float32 spacing per element. Mutation check: clipping MAMBA at 1 instead of 35 leaves gradients and residuals intact and fails the momentum buffers by ~67%.
...
- The repo uses it in place, since the local disk (135 GB free) could not hold a copy. `data/ILSVRC/` (git-ignored) holds symlinks in the config layout: `Data/VID`, `Annotations`, `ImageSets`, and `annotations` → the drive's `Annotations/`. Works only while the drive is mounted. The DET-sample part of 5d's check waits for a few DET images, copied back from Isambard after staging.

**Isambard** (`/projects/b5cs/imagenet_vid/`, the project's shared-dataset area). **Staged 2026-09-16** (job 6618179, 61 min): all five SHA-256 checksums match Hugging Face, and every referenced image exists (vid_train 1,122,397; vid_val 176,126; det_30plus1cls 349,721; 0 missing).
...
- **Stochastic evaluation.** Shuffled frame order and random memory sampling mean eval-level parity needs Python `random` and torch's CPU RNG seeded identically, and single-process runs. At full scale the effect is small: M1 landed 0.01 AP50 from the original.
- **Provenance of the published MAMBA run** (resumed at epoch 3; see Reproduction facts).
- ~~`DataContainer` semantics~~ — replicated by `collate_video_test` / `collate_video_train`, checked against mmcv's collate (5b, 5d).
- **Library versions differ for the data path:** numpy 1.23 (legacy) vs 2.5, Pillow 10.4 vs 12.3. OpenCV is 4.14.0 in both. None of it matters for the data path: 5b and 5d are bit-exact on real images.
