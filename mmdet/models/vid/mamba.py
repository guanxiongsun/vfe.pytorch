...
class MAMBA(BaseVideoDetector):
    ...
    This video object detector is the implementation of `MAMBA
    ...
    def __init__(self,
        ...
        super(MAMBA, self).__init__(init_cfg)
        ...
        assert hasattr(self.detector, 'roi_head'), \
            'MAMBA video detector only supports two stage detector'
    ...
    def forward_train(self,
        ...
        if self.detector.with_rpn:
            ...
            # select topk
            for i, _proposals in enumerate(ref_proposals_list):
                ref_proposals_list[i] = _proposals[:self.detector.roi_head.bbox_head.topk]
    ...
    def extract_feats(self, img, img_metas, ref_img, ref_img_metas):
        ...
        if frame_stride < 1:
            ...
            # first frame -> initialise with adaptive frames
            if frame_id == 0:
                self.memo = Dict()
            ...
            # not first frame -> extract x only
            else:
                ref_x, ref_img_metas = None, None
