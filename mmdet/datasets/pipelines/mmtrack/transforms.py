...
class SeqCropLikeSiamFC:
    ...
    def crop_like_SiamFC(
        self, image, bbox, context_amount=0.5, exemplar_size=127, crop_size=511
    ):
    ...
    def generate_box(self, image, gt_bbox, context_amount, exemplar_size):
        ...
        bbox = np.array(
            [cx - 0.5 * w, cy - 0.5 * h, cx + 0.5 * w, cy + 0.5 * h], dtype=np.float32
        )
    ...
    def __call__(self, results):
        ...
        for _results in results:
            ...
            crop_img = self.crop_like_SiamFC(
                image, gt_bbox, self.context_amount, self.exemplar_size, self.crop_size
            )
            generated_bbox = self.generate_box(
                crop_img, gt_bbox, self.context_amount, self.exemplar_size
            )
...
class SeqShiftScaleAug:
    ...
    def __init__(self, target_size=[127, 255], shift=[4, 64], scale=[0.05, 0.18]):
    ...
    def __call__(self, results):
        ...
        for i, _results in enumerate(results):
            ...
            crop_img, crop_bbox = self._shift_scale_aug(
                image, gt_bbox, self.target_size[i], self.shift[i], self.scale[i]
            )
...
class SeqResize(Resize):
    ...
    def __call__(self, results):
        ...
        for i, _results in enumerate(results):
            # if i > 0 and self.override:
            #     self.override = False       # do not random override for reference frames
            #     for j in range(1, len(results)):
            #         results[j].pop('scale')
            #         if 'scale_factor' in results[j]:
            #             results[j].pop('scale_factor')
...
class SeqResize2(Resize):
    ...
    def __call__(self, results):
        ...
        """
        outs, scale = [], None
        # pop previous scale and scale factor
        for i, _results in enumerate(results):
            # delete scale infos of SeqResize
            if "scale" in _results:
                _results.pop("scale")
                if "scale_factor" in _results:
                    _results.pop("scale_factor")
            # if i > 0 and self.override:
            #     self.override = False       # do not random override for reference frames
            #     for j in range(1, len(results)):
            #         results[j].pop('scale')
...
class SeqMaxSizePad:
    def _pad_img(self, results, size):
        ...
        for key in results.get("img_fields", ["img"]):
            padded_img = mmcv.impad(results[key], shape=size)
    ...
    def __call__(self, results):
        sizes = []
        for _r in results:
            sizes.append(_r["img"].shape[:2])
        all_sizes = np.vstack(sizes)
        max_size = all_sizes.max(axis=0)

        for _r in results:
            self._pad_img(_r, max_size)
...
@PIPELINES.register_module()
class SeqPhotoMetricDistortion:
