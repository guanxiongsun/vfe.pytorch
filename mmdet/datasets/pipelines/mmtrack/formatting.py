        ...
        for i, result in enumerate(results[1:], 1):
            ...
            for key in [
                ...
                value = np.concatenate(
                    (np.full((N, 1), i - 1, dtype=np.float32), value), axis=1
                )
                if i == 1:
                    result[key] = value
                else:
    ...
    def default_format_bundle(self, results):
        ...
        if "gt_semantic_seg" in results:
            ...
            if len(semantic_seg.shape) == 2:
                ...
                semantic_seg = np.ascontiguousarray(semantic_seg.transpose(3, 2, 0, 1))
...
class ReIDFormatBundle:
    ...
    def __call__(self, results):
        ...
        if isinstance(results, list):
            ...
            inputs["img"] = np.stack([_results["img"] for _results in results], axis=3)
